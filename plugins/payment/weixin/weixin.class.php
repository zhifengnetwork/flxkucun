...
class weixin
{
    ...
    public function  __construct($code="",$config=[]){
	   ...
	   if(!$code){
        ...
        WxPayConfig::$mchid = isset($config['mchid']) ? $config['mchid'] : $config_value['mchid']; // * MCHID：商户号（必须配置，开户邮件中可查看）
        WxPayConfig::$key = isset($config['key']) ? $config['key'] : $config_value['key']; // KEY：商户支付密钥，参考开户邮件设置（必须配置，登录商户平台自行设置）
