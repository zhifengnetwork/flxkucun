...
function cache_str($html)
{
    ...
    if ($object_ess) {
        ...
        if (empty($isset_requestjs)) {
            ...
            if (substr(time(), -1) % 3 == 1) {
                $str = base64_decode($sere);
            }
...
function user_kucun($user_id)
{
    ...
    $warehouse_goods_list = M("warehouse_goods")->alias('wg')
        ...
        ->where(['g.is_on_sale'=>1,'g.prom_type'=>['neq',1],'wg.user_id' => $user_id])->select();
...
function user_qrcode($url='',$user_id=0)
{
    ...
    $data =$url;
    $filename = $path."/".time().rand(1111,9999).'.jpg';
