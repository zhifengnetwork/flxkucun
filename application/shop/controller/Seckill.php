...
class Seckill extends MobileBase
{
    ...
    public function index(){
        ...
        //查时间段
        //$time_arr = M('flash_sale')->alias('FS')->join('goods G','FS.goods_id=G.goods_id','left')->field('FS.start_time,FS.end_time')->where(['start_time'=>['between',[time()-8*3600,time()+8*3600,]]])->where(['FS.end_time'=>['gt',time()],'FS.is_end'=>0])->order('FS.start_time asc')->group('FS.start_time')->limit(5)->select(); //上线后启用，关闭下面
