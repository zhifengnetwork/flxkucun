...
class ActivityImg extends Base {

    public function img_set(){

        $info = M('config')->where('inc_type','activity_mg')->cache(true, TPSHOP_CACHE_TIME, 'activity_mg')->column('name,value');
        
        if(request()->isPost()){
            $data = input('post.');
            if($info){
                foreach($data as $key=>$val){
                    M('config')->where('inc_type','activity_mg')->where('name',$key)->update(['value'=>$val]);
                }
            }else{
                foreach($data as $key=>$val){
                    M('config')->insert(['inc_type'=>'activity_mg','name'=>$key,'value'=>$val]);
                }
            }
